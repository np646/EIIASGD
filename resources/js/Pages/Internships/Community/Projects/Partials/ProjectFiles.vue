...
const openUpdateDialog = (id) => {
    ...
    form.project_files_id = id;
};

const uploadFile = () => {
    form.post(
        route("community.project.storeFile", {
            parentId: 2,
        }),
        {
            onSuccess: () => {
...
const deleteFile = async () => {
    const url = route("files.destroyProject", {
...
const fetchItems = async () => {
    try {
        const response = await axios.get(route("api.community.project.projectFiles", project));
