<template>
    <Dialog :visible="modelValue" @update:visible="$emit('update:modelValue', $event)" modal header="Editar" :style="{ width: '30rem' }">
        <form @submit.prevent="updateItem" class="flex flex-col gap-4">
            <div class="field">
                <label for="edit-period">Periodo académico</label>
                <InputText id="edit-period" v-model="form.period" required class="w-full" />
            </div>
            <div class="field">
                <label for="edit-year">Año</label>
                <InputText id="edit-year" v-model="form.year" required class="w-full" />
            </div>
...
const updateItem = async () => {
    loading.value = true;
    try {
        const response = await axios.put(route('api.academicPeriods.update', { period: props.itemData.id }), form.value);
        ...
        toast.add({
            severity: "error",
            summary: "Error",
            detail: "No fue posible actualizar el periodo.",
            life: 3000,
        });
    } finally {
        loading.value = false;
